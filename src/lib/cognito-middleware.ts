...
export async function validateCognitoToken(
  ...
  try {
    console.info("🔍 [COGNITO-MIDDLEWARE] Starting token validation");
    ...
    console.debug(
      "🔍 [COGNITO-MIDDLEWARE] Auth header:",
    ...
    if (authHeader?.startsWith("Bearer ")) {
      ...
      console.info(
        "✅ [COGNITO-MIDDLEWARE] Token extracted from Authorization header"
      );
    } else {
      console.warn(
        "⚠️ [COGNITO-MIDDLEWARE] No Bearer token in Authorization header, trying cookies"
      );
      // Fallback to cookie
      const tokenCookie = request.cookies.get("auth-token");
      token = tokenCookie?.value || null;
      if (token) {
        console.info("✅ [COGNITO-MIDDLEWARE] Token found in cookies");
      }
    }

    if (!token) {
      console.warn("❌ [COGNITO-MIDDLEWARE] No token provided");
      return { isAuthenticated: false, error: "No token provided" };
    }

    console.debug(
      "🔍 [COGNITO-MIDDLEWARE] Token preview:",
    ...
    console.debug(
      "🔍 [COGNITO-MIDDLEWARE] Extracted Cognito user ID:",
      cognitoUserId
    );

    if (!cognitoUserId) {
      console.warn(
        "❌ [COGNITO-MIDDLEWARE] Invalid token format - could not extract user ID"
      );
    ...
    console.debug(
      "🔍 [COGNITO-MIDDLEWARE] Looking up local user for Cognito ID:",
      cognitoUserId
    );
    ...
    if (!localUser) {
      console.warn(
        "❌ [COGNITO-MIDDLEWARE] User not found in local database for Cognito ID:",
        cognitoUserId
      );
      return {
    ...
    console.info("✅ [COGNITO-MIDDLEWARE] Local user found:", {
      id: localUser.id,
...
function extractCognitoUserIdFromToken(token: string): string | null {
  try {
    console.debug("🔍 [TOKEN-EXTRACT] Extracting user ID from JWT token");

    // JWT tokens have 3 parts separated by dots
    const parts = token.split(".");
    console.debug("🔍 [TOKEN-EXTRACT] JWT parts count:", parts.length);

    if (parts.length !== 3) {
      console.warn(
        "❌ [TOKEN-EXTRACT] Invalid JWT format - expected 3 parts, got",
        parts.length
      );
      return null;
    }
    ...
    console.debug("🔍 [TOKEN-EXTRACT] Payload part length:", payload.length);

    const decodedPayload = Buffer.from(payload, "base64url").toString("utf8");
    const claims = JSON.parse(decodedPayload);
    console.debug("🔍 [TOKEN-EXTRACT] Token claims:", {
      sub: claims.sub,
      iss: claims.iss,
    ...
    console.debug("🔍 [TOKEN-EXTRACT] Extracted user ID:", userId);
    return userId;
  } catch (error) {
    console.error(
      "❌ [TOKEN-EXTRACT] Error extracting user ID from token:",
      error
    );
    return null;
  }
...
> {
  ...
  if (
    ...
    !authResult.localUser ||
    !authResult.cognitoUserId
  ) {
    return {
 ...
 */
export async function validateCognitoTokenWithAmplify(
