...
export async function syncCognitoUser(
  ...
  try {
    ...
    if (existingUser) {
      ...
      const updatedUser = await updateUserWithCognitoData(
        existingUser,
        cognitoUser
      );
 ...
 */
async function findUserByCognitoId(
  cognitoUserId: string
): Promise<LocalUser | null> {
  const result = await query("SELECT * FROM users WHERE cognito_user_id = $1", [
    cognitoUserId,
  ]);
 ...
 */
async function createLocalUserFromCognito(
  cognitoUser: CognitoUser
): Promise<LocalUser> {
  const { userId: cognitoUserId, username, email } = cognitoUser;
  ...
  console.info(`✅ Created new local user for Cognito user: ${email}`);
 ...
 */
async function updateUserWithCognitoData(
  ...
  if (!existingUser.cognito_user_id && !existingUser.cognito_email) {
    ...
    console.info(`✅ Updated existing user with Cognito data: ${email}`);
    return result.rows[0];
  }
 ...
 */
export async function getLocalUserByCognitoId(
  cognitoUserId: string
): Promise<LocalUser | null> {
  try {
    const result = await query(
