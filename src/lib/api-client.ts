...
class ApiClient {
  ...
  private async getAccessToken(): Promise<string | null> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.debug(
          `🔍 [API-CLIENT] Token retrieval attempt ${attempt}/${maxRetries}...`
        );
        const session = await fetchAuthSession();

        // Debug session object structure
        console.debug(`🔍 [API-CLIENT] Session object (attempt ${attempt}):`, {
        ...
        if (!token) {
          console.warn(
            `❌ [API-CLIENT] No access token found (attempt ${attempt})`
          );
          if (attempt < maxRetries) {
            const delay = baseDelay * Math.pow(2, attempt - 1); // Exponential backoff: 250ms, 500ms, 1000ms
            console.info(
              `⏳ [API-CLIENT] Retrying token retrieval in ${delay}ms...`
            );
            await new Promise((resolve) => setTimeout(resolve, delay));
            continue;
          }
        ...
        if (accessToken && accessToken.payload.exp) {
          ...
          console.debug(`🔍 [API-CLIENT] Token timing (attempt ${attempt}):`, {
          ...
          if (expirationTime - bufferTime < now) {
            console.warn(
              `🔄 [API-CLIENT] Access token is expired or about to expire (attempt ${attempt})`
            );
            if (attempt < maxRetries) {
              const delay = baseDelay * Math.pow(2, attempt - 1);
              console.info(
        ...
        console.info(
          `✅ [API-CLIENT] Valid access token retrieved successfully on attempt ${attempt}`
        );
        return token;
      } catch (error) {
        console.error(
          `❌ [API-CLIENT] Token retrieval attempt ${attempt} failed:`,
          {
        ...
        if (attempt < maxRetries && isRetryable) {
          const delay = baseDelay * Math.pow(2, attempt - 1);
          console.info(`⏳ [API-CLIENT] Retrying after error in ${delay}ms...`);
          await new Promise((resolve) => setTimeout(resolve, delay));
          continue;
        }
  ...
  async startTracking(
    taskId: string,
    date: string
  ): Promise<{ startTime: number; date: string }> {
  ...
  async getTaskDailyData(
    ...
    const endpoint = `/tasks/${taskId}/daily-data${
